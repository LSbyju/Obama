...
// for the 100% completion function
(function () {
  ...
  function createHiddenCheckbox() {
    const difficultyEl = document.getElementById("difficulty_level");
    if (!difficultyEl) return;

    if (document.getElementById("hidden_wrapper")) {
      const cb = document.getElementById("hidden_checkbox");
    ...
    checkbox.id = "hidden_checkbox";
    checkbox.style.marginRight = "10px";
    checkbox.style.verticalAlign = "text-bottom";
    checkbox.checked = Boolean(window.campaignTrail_temp?.hidden_mode);

    checkbox.addEventListener("change", function () {
      if (!window.campaignTrail_temp) window.campaignTrail_temp = {};
      campaignTrail_temp.hidden_mode = this.checked;

      if (typeof window.enableHiddenMode === "function") {
        try { window.enableHiddenMode(this.checked); } catch (e) { /* ignore */ }
      }

      try {
        document.dispatchEvent(new CustomEvent("hiddenModeToggled", { detail: { enabled: this.checked } }));
      } catch (e) { /* ignore */ }
    });

    label.appendChild(checkbox);
    label.appendChild(document.createTextNode("100% Mode"));

    const note = document.createElement("div");
    note.id = "hidden_label";
    note.style.fontSize = ".8rem";
    note.style.color = "#f0f0f0";
    note.style.marginTop = "4px";
    note.textContent = window.campaignTrail_temp?.hidden_label
      ?? "Congratulations on completing THE OBAMANATION! Please enjoy this exclusive endgame mode.";

    wrapper.appendChild(label);
    wrapper.appendChild(note);

    difficultyEl.appendChild(wrapper);
  }

  function init() {
    const parsed = readUnlockedAch();
    if (!parsed) return; // nothing unlocked or not present
    const required = Object.keys(window.campaignTrail_temp?.achievements || {});
    ...
    if (!unlockedSet) return;

    if (requiredNormalized.every((r) => unlockedSet.has(r))) {
      try { createHiddenCheckbox(); } catch (e) { /* ignore */ }
    }
  }
  ...
  function forceDisplayHiddenMode() {
    try {
      if (!document.getElementById("difficulty_level")) {
        console.warn(
          "Cannot force display: The target element '#difficulty_level' is not in the DOM yet."
        );
        return;
      }
      ...
      console.error("An error occurred while forcing the hidden mode checkbox:", e);
    }
  }
  ...
  const observer = new MutationObserver(() => {
    try {
      if (document.getElementById("difficulty_level") && !document.getElementById("hidden_wrapper")) {
        init();
      }
    } catch (e) { /* ignore */ }
  });
